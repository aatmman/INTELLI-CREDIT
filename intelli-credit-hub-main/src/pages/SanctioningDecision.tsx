...
export default function SanctioningDecision() {
  ...
  const apiMetrics = packData?.data ? [
    { label: "Revenue FY24", value: `₹${packData.data.loan_amount_requested ? (packData.data.loan_amount_requested / 1e7).toFixed(1) + " Cr" : "45.2 Cr"}` },
    { label: "Risk Grade", value: packData.data.risk_grade || "C" },
    { label: "PD Score", value: packData.data.probability_of_default ? `${(packData.data.probability_of_default * 100).toFixed(1)}%` : "12.4%" },
  ] : null;
  ...
  return (
    <DashboardLayout role="sanctioning" roleLabel={getRoleLabel(role)} navItems={NAV} userName={userName}>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {decision && (
          <div
            className={`glass-card text-center py-10 border-2 ${
              decision === "approve"
                ? "border-foreground"
                : decision === "reject"
