...
const shapPositive = [
  { name: "Cash Flow Strength", value: 70 },
  { name: "Collateral Value", value: 55 },
  { name: "Revenue Growth", value: 45 },
];

const shapNegative = [
  { name: "Market Volatility", value: 50 },
  { name: "Industry Risk", value: 35 },
  { name: "Promoter Risk", value: 25 },
];

const policyChecks = [
  { label: "KYC Compliance", status: "PASS" },
  { label: "Net Worth Verification", status: "PASS" },
  { label: "Debt-to-Equity Ratio", status: "FAIL" },
  { label: "Promoter Background Check", status: "PASS" },
  { label: "Legal Pending Status", status: "REVIEW" },
];
...
export default function CreditManagerDecision() {
  ...
  const displayShapPos = apiShapPositive?.length ? apiShapPositive : shapPositive;
  const displayShapNeg = apiShapNegative?.length ? apiShapNegative : shapNegative;
  const displayPolicyChecks = apiPolicyChecks?.length ? apiPolicyChecks : policyChecks;
  const riskGrade = packData?.data?.risk_grade || riskData?.data?.risk_grade || "C";
  ...
  return (
    <DashboardLayout role="credit-manager" roleLabel={getRoleLabel(role)} navItems={NAV} userName={userName}>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {/* ── Decision result ── */}
        {decision && (
          <div className={`glass-card text-center py-10 border-2 ${
            decision === "approve" ? "border-foreground" : "border-foreground/30"
          }`}>
            <div
              className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${
                decision === "approve" ? "bg-foreground text-background" : "bg-muted text-muted-foreground"
              }`}
            >
              {decision === "approve"
