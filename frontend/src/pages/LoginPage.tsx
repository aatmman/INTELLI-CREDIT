...
import { useAuth, ROLE_ROUTES, UserRole } from "@/lib/auth";
...
export default function LoginPage() {
  ...
  const { firebaseLogin, firebaseSignup, login } = useAuth();
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      // Try resolving via Firebase Auth first
      // The flow requested: if they put email/pass, try to sign in
      // If user not found, create account for that role
      try {
        await firebaseLogin(email, password, role);
      } catch (err: any) {
        if (err.code === "auth/user-not-found" || err.code === "auth/invalid-credential") {
          ...
          try {
            ...
            setErrorMsg(signupErr.message || "Failed to create account.");
            setLoading(false);
            return;
          }
        } else {
      ...
      // Complete fallback to demo mode if Firebase isn't configured at all
      console.warn("Firebase Auth failed, falling back to Demo Mode", fallbackErr);
          ...
          <div className="w-full max-w-[380px]">
            ...
            <div className="mb-10">
              ...
              <p className="text-muted-foreground text-sm mt-1" style={{ letterSpacing: "-0.01em" }}>
                Enter your credentials. If you don't have an account, one will be created.
              </p>
            </div>

            <form onSubmit={handleAuth} className="space-y-5">
              {errorMsg && (
                <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-[3px] text-red-600 dark:text-red-400 text-sm mb-4">
                  {errorMsg}
                </div>
              )}
