...
export default function AnalysisWorkspace() {
  ...
  const financialData: any[] = finApiData?.data || [];
  const gstBarData: any[] = gstApiData?.data?.bar_data || [];
  const bankingBarData: any[] = bankApiData?.data?.bar_data || [];
  const balanceData: any[] = bankApiData?.data?.balance_trend || [];
  const itcData: any[] = gstApiData?.data?.itc_data || [];
  const researchFindings: any[] = researchApiData?.data || [];
  const timelineEvents: any[] = timelineApiData?.data || [];
  const camSections: any[] = [];
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-1.5">
              {["All", "News", "MCA", "e-Courts", "Sector", "RBI"].map((f) => (
                <button
                  key={f}
                  className={`h-7 px-3 text-xs font-mono rounded-[3px] transition-colors ${f === "All"
            ...
            <div className="space-y-2.5">
              {researchFindings.map((f, i) => (
                <div
                  key={i}
                  className={`glass-card border-l-2 ${f.severity === "HIGH" ? "border-foreground" : f.severity === "MED" ? "border-foreground/40" : "border-foreground/15"
                    }`}
                >
